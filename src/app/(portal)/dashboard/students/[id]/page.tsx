...
import { Card, CardHeader, CardBody, Button, Badge, PageLoader, Modal, Input, Select, Alert, Avatar } from '@/components/portal/ui';
...
import { Student, Batch, FeePayment, AttendanceRecord } from '@/types/portal';
...
export default function StudentDetailPage() {
  ...
  useEffect(() => {
    async function fetchStudent() {
      ...
      const enrichedStudent: StudentWithDetails = {
        ...studentData,
        batches: studentBatches?.map((sb: any) => sb.batches).filter(Boolean) as Batch[],
      ...
      setEditForm({
        ...
        batch_ids: studentBatches?.map((sb: any) => sb.batch_id) || [],
      });
      setLoading(false);
    }
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await (supabase.from('student_batches') as any).delete().eq('student_id', student.id);
      if (editForm.batch_ids.length > 0) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await (supabase.from('student_batches') as any).insert(
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Delete student
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { error: deleteError } = await (supabase.from('students') as any).delete().eq('id', student.id);
