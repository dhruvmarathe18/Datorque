...
import { StudyMaterial, Batch } from '@/types/portal';
    ...
    async function fetchData() {
      ...
      if (filterBatch !== 'all') {
        ...
        if (batchesData && batchesData.length > 0) {
          materialsQuery = materialsQuery.in('batch_id', batchesData.map((b: any) => b.id));
        }
      }
  ...
  const columns = [
    {
      ...
      render: (material: StudyMaterialWithBatch) => (
        <div className="flex items-center gap-3">
          <span className="text-2xl">{getFileTypeIcon(material.file_type ?? null)}</span>
          <div>
    ...
    },
    {
      ...
      header: 'Subject',
      render: (material: StudyMaterialWithBatch) => (
        <Badge variant="default">{material.subject || 'General'}</Badge>
      ),
    },
    {
      key: 'batch',
