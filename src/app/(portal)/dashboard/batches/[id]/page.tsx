...
import { Card, CardHeader, CardBody, Button, Badge, PageLoader, Modal, Input, Textarea, Alert, Avatar, DataTable } from '@/components/portal/ui';
import { formatDate, formatTime } from '@/lib/utils';
import { ArrowLeft, Edit, Trash2, Users, Clock, Calendar, BookOpen, Plus, X } from 'lucide-react';
import Link from 'next/link';
import { Batch, Student, Staff } from '@/types/portal';
...
export default function BatchDetailPage() {
    ...
    async function fetchBatch() {
      ...
      if (batchStaff && batchStaff.length > 0) {
        const { data: staffUsers } = await supabase
          .from('users')
          .select('*')
          .in('id', batchStaff.map((bs: any) => bs.staff_id));
        staffData = staffUsers || [];
      }

      const enrichedBatch: BatchWithDetails = {
        ...batchData,
        students: studentBatches?.map((sb: any) => sb.students).filter(Boolean) as Student[],
        staff: staffData,
      };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (supabase.from('student_batches') as any).delete().eq('batch_id', batch.id);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (supabase.from('batch_staff') as any).delete().eq('batch_id', batch.id);

      // Delete batch
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { error: deleteError } = await (supabase.from('batches') as any).delete().eq('id', batch.id);
            ...
            {batch.subjects && batch.subjects.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {batch.subjects.map((subject) => (
                  <Badge key={subject} variant="info" className="text-sm">
                    {subject}
                  </Badge>
                ))}
              </div>
