...
import { Card, CardHeader, CardBody, Button, Badge, PageLoader, Alert, Modal, Input, Select } from '@/components/portal/ui';
...
export default function InstituteDetailsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchInstituteDetails() {
      ...
      const [studentsRes, batchesRes, adminsRes] = await Promise.all([
        ...
        (supabase.from('students') as any).select('*', { count: 'exact', head: true }).eq('institute_id', params.id),
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (supabase.from('batches') as any).select('*', { count: 'exact', head: true }).eq('institute_id', params.id),
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (supabase.from('users') as any).select('*', { count: 'exact', head: true }).eq('institute_id', params.id).eq('role', 'coaching_admin'),
      ]);
  ...
  const handleDelete = async () => {
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { error } = await (supabase.from('institutes') as any).delete().eq('id', params.id);
