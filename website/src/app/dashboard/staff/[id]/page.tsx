...
import { Card, CardHeader, CardBody, Button, Badge, PageLoader, Modal, Input, Alert, Avatar } from '@/components/ui';
import { formatDate } from '@/lib/utils';
import { ArrowLeft, Edit, Trash2, User, Phone, Mail, Briefcase, BookOpen, Plus, X } from 'lucide-react';
import Link from 'next/link';
import { Staff, Batch } from '@/types';
...
export default function StaffDetailPage() {
  ...
  useEffect(() => {
    async function fetchStaff() {
      ...
      const enrichedStaff: StaffWithDetails = {
        ...staffData,
        batches: batchStaff?.map((bs) => bs.batches).filter(Boolean) as Batch[],
      };
      ...
      setEditForm({
        ...
        batch_ids: batchStaff?.map((bs) => bs.batch_id) || [],
      });
      setLoading(false);
    }
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await supabase.from('batch_staff').delete().eq('staff_id', staff.id);
      if (editForm.batch_ids.length > 0) {
        await supabase.from('batch_staff').insert(
